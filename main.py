...
from models.ProposedKANmodel import ProposedKAN
...
def DUQKANTest():
    ...
    numClasses = 3
    trainLoader,testLoader,falseLoader1,falseLoader2,falseLoader3,falseLoader4 = loadAllDataloaders(root,True if numClasses==2 else False)
    ...
    #3l
    archs = [[3,16,numClasses],[3,32,numClasses],[3,64,numClasses],[3,8,4,numClasses],[3,16,8,numClasses],[3,32,16,numClasses],
             [3,16,16,16,numClasses],[3,16,16,32,numClasses],[3,16,32,16,numClasses],[3,32,16,16,numClasses],[3,16,32,64,numClasses]]
    archs = [[3,16,32,3]] # conf
    #archs = [[3,8,4,2]] # conf
    #archs = [[3,128,3]]
                    ...
                    for ls in lengthScales:
                        ...
                        for gamma in gammas:
                            ...
                            for _ in range(numTestModels):
                                ...
                                model = ProposedKAN(ar,num_grids=grids,base_activation=baseActivation).cuda()
...
def DUQTest():
    ...
    netType = 'duq'
    architecture = [3,32,16,3]
    lr = 0.1
    wd = 1e-4
    gp = 0.25
    epochs = 120
    featureExtractor = MLP(architecture[:-2],activationFunction=nn.ReLU())
    model = DUQ(featureExtractor=featureExtractor,numClasses=architecture[-1],centroidSize=architecture[-2],
                featureSize=architecture[-3],std=1,sigma=1).to(device)

    lossFunction = nn.CrossEntropyLoss(reduction="mean")

    optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=wd)
    scheduler = ReduceLROnPlateau(optimizer, 'min', patience=5)
    trainAcc,trainLoss,testAcc,testLoss,aurocs = networkTrain(netType,model,optimizer,scheduler,
                                                             lossFunction,trainLoader,testLoader,falseloaders,
                                                             numClasses,gp,epochs)
