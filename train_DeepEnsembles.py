...
def main(architecture,
    ...
    for _ in range(model_num):
        models = [SoftmaxModel(architecture,activationFunction=nn.ReLU()).cuda() for _ in range(DE_num)]

        optimizers = [torch.optim.SGD(
            models[i].parameters(), lr=learning_rate) for i in range(DE_num)]
        schedulers = [ReduceLROnPlateau(optimizers[i], 'min', patience=5) for i in range(DE_num)]

        trainAcc,trainLoss,testAcc,testLoss,auroc = DeepEnsambleTrain(models,optimizers,schedulers,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help='Number of models per Deep Ensambles',
    )
